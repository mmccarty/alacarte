...
before_filter :current_page, :only =>[ :edit_reserves, :edit_databases,:update_databases, :add_databases, :sort, :toggle_columns,:add_modules,:edit_module,:update_module, :new_module, :remove_module, :send_url]
before_filter :altered_pages, :except => [:index, :create, :sort, :retrieve_reserves, :destroy ]
     ...
     #read and set the variables 
      ...
      #only send a subset of @pages to the view
       ...
       if request.xhr? 
          ...
          @page.update_resource(params[:mod_list], @user.resources)
          redirect_to :action => "edit", :id => @page.id
       end
  end
...
def edit_reserves
    @ecurrent = 'current'
     begin
       ...
       if @mod.course_title.blank? #there was not more than one course when mod was created 
